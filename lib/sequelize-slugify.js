...
class SequelizeSlugify {
  slugifyModel(model, options) {
    ...
    const handleSlugify = async function (instance, sequelizeOptions, forceGenerateSlug = true) {
      // current slug value
      ...
      const transaction =
        slugifyOptions.passTransaction === true ? sequelizeOptions.transaction : null;

      // check if any of the fields used to build the slug have changed
      const sourceChanged = slugifyOptions.source.some(function (slugSourceField) {
        return instance.changed(slugSourceField);
      });

      // check if slug column has changed
      const slugChanged = instance.changed(slugColumn);

      // check if slug changed manually
      if (slugChanged && slugValue) {
        const options = (slugifyOptions && slugifyOptions.slugOptions) || DEFAULT_SLUG_OPTIONS;

        const slugifiedValue = slug(instance[slugColumn], options);

        slugValue = await addNumericSuffix(slugifiedValue, transaction);
      ...
      if (!forceGenerateSlug && !sourceChanged) {
      ...
      slugValue = slugifySourceFields(instance, slugifyOptions.source);

      // documented behaviour. Failure to slugify returns null
      if (slugValue === '' && slugifyOptions.allowNull) {
    ...
    const slugifySourceFields = function (instance, sourceFields) {
      const slugParts = sourceFields.map(function (slugSourceField) {
        const i = instance[slugSourceField];
        return slugifyOptions.lang ? i[slugifyOptions.lang] : i;
      });
    ...
    const addSourceSuffix = async function (instance, slugValue, suffixFields, transaction) {
      ...
      for (let i = 0; i < suffixFields.length; i++) {
        ...
        const newSlugSection = slugifySourceFields(instance, [suffixFields[i]]);
